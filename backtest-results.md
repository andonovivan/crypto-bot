...
- **Live trailing uses Binance's native `TRAILING_STOP_MARKET`** (added after the original write-up). `BinanceExchange::openTrailingStop()` POSTs one algo order at entry with `activationPrice = entry × (1 ∓ trailing_tp_arm_pct/100)` and `callbackRate = trailing_tp_trail_pct`; Binance tracks the extreme server-side and emits the fill via `ORDER_TRADE_UPDATE` like any other bracket. No cancel/replace polling. Dry-run still uses `maybeTrailStop()` for parity (the replay exchange has no order matching engine).
