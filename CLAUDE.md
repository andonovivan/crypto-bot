...
- `HistoricalReplayExchange` — offline backtest exchange. Loads `kline_history` rows into memory, services `getPrice` / `getKlines` / `getFuturesTickers` against a movable clock (`setClock($unixMs)`), synthesizes order-book top from the current price (±1bp), and stubs out order placement / `getOrderStatus` / listenKey as no-ops. Uses the same DB `Position`/`Trade` rows (with `is_dry_run=true`) so account balance and open-position tracking match DryRunExchange. **Shares two friction mechanics with DryRunExchange** (added 2026-05): `openShort/closeShort/openLong/closeLong` apply `dry_run_market_slippage_bps` adverse slippage via the same `applyMarketSlippage()` helper, and `calculateQuantity()` enforces `MIN_NOTIONAL` against the real exchange's cached `getExchangeInfo()` (via `setRealExchange()`) — returning 0 routes through `TradingEngine`'s quantity≤0 path and writes a Failed row. Intentionally does NOT mirror `dry_run_maker_fill_rate` or `dry_run_bracket_fail_rate`: the first adds stochastic noise without changing the strategy's edge characterization (and would just shift some entries from maker→taker fees, captured well enough by leaving all backtest entries as `LIMIT_MAKER`-priced); the second fires too rarely at default 0.01 to meaningfully shift hundreds-of-trades samples. **Only used during `bot:backtest`** — bound into the container via `app()->instance(ExchangeInterface::class, $replay)`. See the Backtest Harness section.
...
| `app/Services/Exchange/HistoricalReplayExchange.php` | Offline replay exchange for `bot:backtest` — serves `kline_history` rows against a movable clock, stubs out order APIs, writes DB rows with `is_dry_run=true`. Supports fixed-sizing mode (`setFixedSizing`) and symbol-scoped probe prices (`setProbePrice`/`clearProbePrice`) for intra-bar SL/TP simulation. Applies the same `dry_run_market_slippage_bps` slippage and `MIN_NOTIONAL` rejection as `DryRunExchange` (skips post-only fill rate and bracket-fail rate; see Backtest Harness > Friction modeled in backtest). |
...
**Failed positions**: `Position` rows with `status=Failed` + `error_message` surface entry rejections (Binance refused the order, bracket placement failed, quantity rounded to zero, etc.) so they're visible in the dashboard rather than log-only. `Position::open()` scope excludes them, so they don't count toward `max_positions` or the one-per-symbol invariant. Written by `TradingEngine::recordFailedEntry()` from three paths: (1) the `placeEntryOrder` exception path, (2) the post-entry/bracket-failure path (including the "UNPROTECTED — close also failed" case), and (3) the **quantity-zero guard** in `openShortInternal` / `openLongInternal` — fires when `calculateQuantity()` returns 0 because notional fell below the symbol's `LOT_SIZE` minQty or `MIN_NOTIONAL` (matches the live Binance `-4164` rejection that small accounts hit on high-min symbols). A Failed row also blocks new entries on the same `(symbol, strategy_key)` for `strategy.<key>.failed_entry_cooldown_minutes` (default 0 = disabled).
...
### Friction modeled in backtest (post 2026-05 reality pass)

- **Market-order slippage**: `openShort/closeShort/openLong/closeLong` shift adversely by `dry_run_market_slippage_bps` (default 3 bps). Backtest entries are mostly `openShortLimit` (LIMIT_MAKER, no slip) so the entry side is unaffected; closes are always MARKET so they always pay slippage. Critically, `TradingEngine::closePosition` applies slippage on top of caller-provided trigger prices (SL/TP/expiry hits) too — without this fix the close-side slippage was silently bypassed because backtest closes always pass the explicit trigger, not `null`. With it on, the empirical drag matches the math (~$3/close at 3 bps × $10K notional).
- **MIN_NOTIONAL rejections**: `calculateQuantity` returns 0 when `qty × price < minNotional` (looked up from the real exchange's cached `getExchangeInfo`). At $10K backtest notional virtually all USDT perps clear; below ~$100 notional you'll see Failed rows for the high-min meme perps, matching live `-4164` behavior.

### Known limitations

- Funding is zeroed (no historical funding data in `kline_history`).
- Post-only limit entries (`openShortLimit`) assume immediate fill at limit — no maker queue simulation, no `dry_run_maker_fill_rate` randomization. Effectively means backtest treats every post-only attempt as a successful maker fill (best-case fee structure).
- Bracket placement failures (`dry_run_bracket_fail_rate`) are not simulated in backtest — at default 0.01 they fire too rarely (~0.02% post-retry) to matter on hundreds-of-trades samples.
- Order-book top is synthesized from the current price (±1bp), so post-only spreads are artificial.
- `calculateQuantity()` uses generic 6-decimal rounding instead of live LOT_SIZE (`minNotional` is enforced; step-size precision is approximated).
